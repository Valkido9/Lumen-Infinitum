...
| `abilities.html` | 能力档案 | 133 个能力条目、四维筛选、排序、六维雷达图 |
...
- 从源文档 `永恒流光能力设定集（2026.6.10）` 解析生成，共 133 个能力条目
...
### 第 24 次更新 — 大后期新能力「帝尊之心」「飞升图腾」+ 世界设定「世界的宏愿」更新
**提交**: `（本次）`
- **新增「帝尊之心」(ab131)**：持有者布格弥，CRT 15.2，面板 `[C,B,EX,D,A,A]`。时间线 `尤克特勒夏/芬奈法拉`（新词表值，复合时间线）、院区 `真理院`、阵营 `芬奈法拉战线|巫魔天国`（`巫魔天国`为新值）、初登场 `大后期`（新卷目值）。神器能力（`sz:1`，金色边框 + 神器能力徽章 + `SHENQI_DESC` 描述框），攻城级（|CRT|≥14 自动触发）。描述中「令其主人瞬间掌握……让其永不疲倦、永不衰弱」整段用 `{R}` 标红（源文档要求），nt 带 `*`（星号段落框惯例）。
- **新增「飞升图腾」(ab132)**：持有者珀希，CRT 15.7，面板 `[EX,A,D,A,C,EX]`。时间线 `其他时间线`（新词表值）、院区 `生命院`、阵营 `死神首席`、初登场 `大后期`。神器能力（`sz:1`）。描述含 `【冥荒】/【不朽】/【超凡】/【咒令】` 分节；`【恶意】`、`【恶意图腾】`、`致命伤害`、复生夺魂整句标红（`{R}`）。
- **更新「世界的宏愿」(ab130)**：描述措辞修正——「父母拳打脚踢」→「拳脚相加」、「周围的所有曾经伤害过自己的人」→「周围的所有人」；引文「把…回到」→「让…回到」；原有 `{R}` 标红（「一切希冀」、各「希望」、末句）全部保留。标签不变（其他 / 未知 / 希冀院 / 前传）。
- **标签词表扩展**（`abilityTags`）：`camps` 新增 `巫魔天国`；`tl` 新增 `其他时间线`、`尤克特勒夏/芬奈法拉`；`vol` 新增 `大后期`。能力总数 131 → 133。

### 第 25 次更新 — 抽屉宽度翻倍 + 名词/时间线抽屉化 + 多时间线 tag + 可乐文案 + 灼晦引文
**提交**: `（本次）`
- **抽屉宽度翻倍**：`.char-drawer` 宽 `360px` → `720px`（五页同步）。
- **名词解释 / 时间线卡片 PC 抽屉化**：`openTerm()` / `openTimeline()` 重构为与 `openChar()` 同构——PC（≥1280px）内容写入右侧抽屉 `#termDrawerContent`/`#tlDrawerContent`（`.modal.char-drawer-inner timeline-modal`），窄屏回落居中弹窗；新增 `closeDrawer(id)` / `closeFlap(modalId, drawerId)`（抽屉开则关抽屉、否则关弹窗），关闭按钮与 `tl-jump` 改用 `closeFlap`；resize 跨断点缩窄时 `['charDrawer','termDrawer','tlDrawer']` 一并自动关闭；Escape 补关两个新抽屉。三抽屉共用同一右侧滑出区域。
- **多时间线 tag 语义**：`parseTg(a)` 的 `tl` 字段改为 `'|'` 分隔数组（与 `camps` 同构），`matchesFilter` 用 `t.tl.some(x => abilityFilter.tl.includes(x))` 匹配——一个能力可同时挂多条时间线。**布格弥 ab131 帝尊之心** `tg` 由 `尤克特勒夏/芬奈法拉` 改为 `尤克特勒夏|芬奈法拉`（表示同时属于两线，而非单个复合 tag）；`abilityTags.tl.values` 移除复合值、新增独立 `尤克特勒夏`。`buildAbilityCard` 内 `parseTg(a)` 只读 `camps`/`ward`，不受影响。
- **角色筛选同步**：`characters.html` 的 `parseCharTg`/`matchesCharFilter` 同样改为数组拆分 + `.some()`，与能力档案框架保持一致。
- **可乐文案修正**：正文「沃克加德的新奇饮料——<span…>可乐</span>」→「柯洛雯的新奇饮料——可乐」，删除失效的蓝色 `term-link` 超链接（无 `cola:` 词条定义，原为 no-op），并移除柯洛雯时间线卡片中失效的 `settings: ['cola']` 项。已检查 `docs/永恒流光故事讲解.html` 等处——可乐其余引用均已正确归因柯洛雯公司，无需改动。
- **帝尊之心描述合并**：ab131 红色段中「为其主人提供无穷无尽的、能够永恒释放任意魔法而永不枯竭的耐力、体能与魔力；让其永不疲倦、永不衰弱」合并为「让其主人永不疲倦、永不衰弱」。
- **灼晦引文后缀**：ab130 世界的宏愿引文统一为「……我们可以让世界的一切回到它本应是的模样。\n——灼晦」（历史「把世界的一切」写法在 characters/index/world 三页一并修正，幂等脚本处理 把/让 两种写法）。
- **临时脚本**：`scripts/batch_update_25.py`（纯本地一次性工具，含逐文件计数断言 + EOL 保留 + 幂等跳过）已在使用后删除，不提交。

### 第 26 次更新 — 16 个能力初登场卷目标签修正（据审阅批注）
**提交**: `（本次）`
- **据审阅导出 16 条批注**修正 `abilityData` 各能力 `tg` 字段的**卷目段**（第 4 段），5 页（index/sidetory/world/abilities/characters）同步，共 80 处替换：
  - ab4 `流光`（云略/修瓦尔扎）：序幕 → **第一卷**
  - ab8 `间力乱流`（红猫）、ab9 `狭明万象`（靖珏）：序幕 → **第二卷**
  - ab34 `暮蓝隙行`（恪钴）、ab118 `恶血天堂`（十七）：第一卷 → **序幕**（修正第 11 次更新挪卷）
  - ab61 `创世之耀`、ab62 `日昳散`、ab63 `装甲大师`（初代时之秩序）、ab65 `电照掣集`（杏子）、ab66 `无穷酒馆`（奇比）、ab68 `西格玛谜题`（Twikyo）：序幕 → **第四卷**；ab64 `元始弹`（莱诺）：第一卷 → **第四卷**
  - ab82 `疾苦咒附`（道杰斯）、ab84 `凝魂清焰`（鸿荧）：第五卷 → **第三卷**
  - ab97 `暗影龙蜥` 解放阶段（契尔文孙）：第五卷 → **第五卷最终战**（与 ab98 同款最终战标签；ab96 首阶段维持第五卷）
  - ab127 `充能星弹`（阿弗加）：第二卷 → **第一卷**
